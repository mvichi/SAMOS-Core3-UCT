...
\begin{frame}{Hands-on tutorial}
    \begin{itemize}
    \item \textbf{\texttt{jupyter lab}}\\ an  Integrated Development Environment (IDE) for python notebooks \\(Sec. 3.4.2 on Amathuba)
...
\column{8cm}
\begin{itemize}
    \item \small{First download from \href{https://github.com/mvichi/SAMOS-Core3-UCT/tree/main/resources}{\textbf{the repository}} the \texttt{shell-lesson-data.zip} file and decompress it into your Download folder.}
    \item \small{From the same Anaconda Prompt window, we can also launch the application without using Navigator: \texttt{> jupyter lab} (enter)}
    \item \small{launch the terminal from Jupyter Lab and test the following commands:}
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=1.\linewidth]{lectures/figures/tree.png}
\end{figure}
\end{columns}
